...
class ConfigModel:
    # Git
    Repository: str = "https://gitee.com/pattering-rain/yys.git"
    Branch: str = "master"
...
class DeployConfig(ConfigModel):
    ...
    def read(self):
        ...
        if self.config["Repository"].startswith("https://e.coding.net/"):
            self.config["Repository"] = "https://gitee.com/pattering-rain/yys.git"
    ...
    def execute(self, command, allow_failure=False, output=True, silent=False):
        ...
        try:
            result = subprocess.run(
                command,
                # Windows can launch executable command lines directly. Avoid
                # cmd.exe here so updater startup does not depend on a shell
                # process successfully initializing.
                shell=os.name != 'nt',
