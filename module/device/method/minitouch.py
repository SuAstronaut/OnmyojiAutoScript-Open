...
class Minitouch(Connection):
    ...
    @retry
    def press_down(self, x, y, contact=0, pressure=100):
        """
        按下操作，调用 minitouch 的 down 命令。

        Args:
            x (int): 按下的 x 坐标
            y (int): 按下的 y 坐标
            contact (int): 触摸点编号，默认为 0
            pressure (int): 压力值，默认为 100
        """
        builder = self.minitouch_builder
        builder.down(x, y, contact=contact, pressure=pressure).commit()
        self.minitouch_send()

    @retry
    def press_up(self, contact=0):
        """
        抬起操作，调用 minitouch 的 up 命令。

        Args:
            contact (int): 触摸点编号，默认为 0
        """
        builder = self.minitouch_builder
        builder.up(contact=contact).commit()
        self.minitouch_send()

    @retry
    def swipe_minitouch_1(self, p1, p2, duration):
