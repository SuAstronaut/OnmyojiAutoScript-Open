...
class CaptureStd:
    ...
    @staticmethod
    def _valid_fileno(stream):
        """返回 stream 的真实文件描述符; 无控制台/被重定向为空(GUI、pythonw、stdout=None)时返回 None"""
        if stream is None:
            return None
        try:
            fd = stream.fileno()
        except (AttributeError, OSError, ValueError):
            # io.UnsupportedOperation 继承自 OSError/ValueError, 一并覆盖
            return None
        return fd if isinstance(fd, int) and fd >= 0 else None

    def __enter__(self):
        # GUI/pythonw/无控制台时 sys.stdout 可能为 None 或无有效 fileno, 无法重定向 fd,
        # 此时跳过捕获直接放行, 避免 "'NoneType' object has no attribute 'fileno'" 之类崩溃
        self._noop = False
        fdout = self._valid_fileno(sys.stdout)
        fderr = self._valid_fileno(sys.stderr)
        if fdout is None or fderr is None:
            self._noop = True
            return self
...
class NemuIpc(Connection):
    # 由 Device.wait_until_ready() 在轮询期间置 True.
    # 模拟器启动过程中连不上是预期现象, 此时降级日志, 只有等待超时才真正报错.
    waiting_emulator_ready = False

    @cached_property
    def nemu_ipc(self) -> NemuIpcImpl:
        """
        Initialize a nemu ipc implementation
        通过已保存的模拟器安装路径(emulatorinfo_path) + serial 推断的实例ID 进行初始化.
        连接失败(模拟器信息不正确 / 尚未完全启动)会抛出 RequestHumanTakeover,
        上层 nemu_ipc_available() 会捕获并返回 False, 便于"等待模拟器就绪"时反复重试.
        """
        log = logger.info if self.waiting_emulator_ready else logger.error

        # with E:\ProgramFiles\MuMuPlayer-12.0\shell\MuMuManager.exe
        # installation path is E:\ProgramFiles\MuMuPlayer-12.0
        emulatorinfo_path = self.config.script.device.emulatorinfo_path
        if not emulatorinfo_path:
            logger.error('未配置模拟器安装路径(emulatorinfo_path)，无法使用 NemuIpc')
            raise RequestHumanTakeover

        folder = str(Path(emulatorinfo_path).parent.parent)
        index = serial_to_id(self.config.script.device.serial)
        if index is None:
            logger.error(f'无法从 serial={self.config.script.device.serial} 推断 MuMu 实例ID，无法使用 NemuIpc')
            raise RequestHumanTakeover
        ...
        try:
            ...
            logger.error(e)
            logger.error('MuMu 版本过低或不兼容，无法使用 NemuIpc')
            raise RequestHumanTakeover
        except NemuIpcError as e:
            log(e)
            log('NemuIpc 连接失败：模拟器信息不正确或尚未完全启动')
            raise RequestHumanTakeover
