...
class PlatformWindows(PlatformBase, EmulatorManager):
    ...
    def _emulator_start(self, instance: EmulatorInstance):
        ...
        """
        emulator_window = self.config.script.device_2.emulator_window
    ...
    def _wait_mumu_stop(self, instance: EmulatorInstance):
        """
        MuMu shutdown 命令提交后立即返回, 实例(尤其安卓15)完全退出仍需一段时间,
        轮询 MuMuManager info 确认进程已退出, 避免关闭未完成就再次启动导致失败.

        Returns:
            bool: True if stopped, False if timeout
        """
        from module.device.execute_util import execute_emulator
        mumu_id = instance.MuMuPlayer12_id
        if mumu_id is None:
            return True
        console = Emulator.single_to_console(instance.emulator.path)
        logger.info(f'等待模拟器关闭确认: {instance.name}')
        timeout = Timer(30).start()
        while 1:
            try:
                info = execute_emulator([console, 'info', '-v', str(mumu_id)])
            except Exception as e:
                logger.warning(f'查询模拟器状态失败: {e}')
                info = None
            if isinstance(info, dict) and info.get('is_process_started') is False:
                logger.info(f'模拟器已关闭: {instance.name}')
                return True
            if timeout.reached():
                logger.warning(f'等待模拟器关闭确认超时: {instance.name}')
                return False
            Timer(1).start().wait()
    ...
    def _emulator_stop(self, instance: EmulatorInstance):
        ...
        if instance == Emulator.MuMuPlayer:
            ...
            return self._wait_mumu_stop(instance)
        elif instance == Emulator.LDPlayerFamily:
    ...
    def _emulator_function_wrapper(self, func: callable):
        ...
        try:
            result = func(self.emulator_instance)
