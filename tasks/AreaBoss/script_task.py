...
class ScriptTask(AreaBossAssets, GeneralBattle, SwitchSoul):
    """ 地域鬼王 """
    def run(self) -> bool:
        """
        运行脚本
        ...
        # 挑战鬼王次数
        boss_number = self.config.area_boss.boss.boss_number

        battle_list = [self.I_BATTLE_1, self.I_BATTLE_2, self.I_BATTLE_3]
        for item in battle_list:
            if self.current_count >= boss_number:
                break
            self.boss_fight(item, flag=flag)

        self.set_next_run(task='AreaBoss', success=True, finish=False)
        if self.current_count < boss_number:
            self.push_notify(content=f'战斗次数{self.current_count}小于要求次数{boss_number}, 请查看')
        # 以抛出异常的形式结束
        raise TaskEnd
    ...
    def boss_fight(self, battle: RuleImage, ultra: bool = False, fileter_open: bool = True, flag: bool = False) -> bool:
        ...
        # 如果已经打过该BOSS,直接跳过不打了
        if not self.wait_until_appear(self.I_AB_GROUP_RANK_NONE, wait_time=3):
            logger.info("该BOSS已经打过了,已经获得小组排名")
            self.ui_click_until_disappear(self.I_BACK_RED, interval=3)
            return True
        ...
        if not self.start_fight():
            result = False
            logger.warning("地域鬼王战斗失败")
        self.wait_until_appear(self.I_BACK_RED)
        self.ui_click_until_disappear(self.I_BACK_RED, interval=1)
        return result
    ...
    def start_fight(self) -> bool:
        ...
        while 1:
            ...
            if self.appear_then_click(self.I_FIRE, interval=1):
                continue
            if not self.appear(self.I_BACK_RED):  # 如果这个红色的关闭不见了才可以进行继续
                break
    ...
    def get_hot_in_reward(self):
        ...
        # 定义处理流程参数
        process_steps = [
            # (photo_param, swipe_times)
            (self.C_AB_BOSS_REWARD_PHOTO_1, 0),
            (self.C_AB_BOSS_REWARD_PHOTO_2, 0),
            (self.C_AB_BOSS_REWARD_PHOTO_3, 0),
            (self.C_AB_BOSS_REWARD_PHOTO_MINUS_2, 3),
            (self.C_AB_BOSS_REWARD_PHOTO_MINUS_1, 3)
        ]

        for photo_param, swipe_times in process_steps:
            self.open_filter()
    ...
    def get_num_challenge(self, click_area):
        ...
        if self.open_boss_detail(click_area, 3):
    ...
    def open_boss_detail(self, battle: RuleImage, try_num: int = 3) -> bool:
        ...
        while try_num > 0:
            ...
            if self.wait_until_appear(self.I_BACK_RED, wait_time=3):
                # 打开鬼王详情界面失败,直接返回
                return True
            try_num -= 1
        return False
