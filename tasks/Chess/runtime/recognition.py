...
class ChessRecognitionMixin(_ChessTaskBase):
    ...
    def _load_hand_template_folder(
        ...
        logger.debug(f'已加载 {len(rules)} 个棋局 {folder} 手牌模板')
        return rules
        ...
        for name, config in entries.items():
            for filename in config.get(image_field, ()):
                ...
                if not file.exists():
                    logger.warning(
                        f'棋局策略图片缺失: '
                        f'阵容={self.get_lineup_strategy()["key"]}, '
                        f'名称={name}, 文件={filename}'
                    )
                    continue
    ...
    def shikigami_shop_rules(self) -> list[tuple[str, RuleImage]]:
        ...
        logger.debug(f'已加载 {len(rules)} 个棋局商店头像模板')
        return rules
    ...
    def all_shikigami_shop_rules(self) -> list[tuple[str, RuleImage]]:
        ...
        logger.debug(f'已加载 {len(rules)} 个全部棋局商店头像模板')
        return rules
    ...
    def _confirm_unknown_hand_card(
        ...
        for confirmation in range(1, self.UNKNOWN_SELL_CONFIRM_FRAMES + 1):
            ...
            if latest['type'] != 'unknown':
                logger.debug(
                    '重复识别后保护棋局手牌: '
                    f'类型={latest["type"]}, 名称={latest["name"]}'
                )
                return None
