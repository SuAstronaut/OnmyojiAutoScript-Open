...
class CostumeConfig(BaseModel):
    ...
    # 庭院
    costume_main_type: bool = Field(default=True, title='已自动适配以下庭院',
                                    description="织梦莲庭、琼夜淬光、烬夜韶阁、笔墨山河、枫色秋庭、暖池青苑、盛夏幽庭、\n"
                                                "远海航船、结缘神社、望月幽庭、鏖刀禁府、龙吟溯玉、云景阆苑、琼月华宫、\n"
                                                "绘世洞天、天穹之境、绮梦茨庭、茨心乐园、\n"
                                                "======= 注意 =======\n"
                                                "枫色秋庭 【没有宠物屋】\n"
                                                "远海航船、结缘神社 【町概率被式神挡住】\n")
    ...
    # 战斗主题
    costume_battle_type: bool = Field(default=True, title='已自动适配以下战斗主题',
                                      description="雅乐之邦、蝶寻花踪、凛霜寒雪、春缕含青、祥夜幽芳、桂馥金秋、笼梦之境、\n"
                                                  "莲华圣域、流焰蝶舞、藏金台阁、藏金台阁、招财纳福、登云问翠、茸茨跃动、\n"
                                                  "======= 注意 =======\n"
                                                  "选择简约主题 【不支持怀旧】\n")

    # 结界皮肤
    costume_realm_type: bool = Field(default=True, title='已自动适配结界皮肤', description="鲤鱼旗不支持\n")
    # 庭院主题
    # costume_theme_type: ThemeType = Field(default=ThemeType.COSTUME_THEME_DEFAULT, description='costume_theme_type_help', hidden=True)
    # 签到主题
    costume_sign_type: bool = Field(default=True, title='已自动适配签到主题', description="庭院任务一键领取\n")
