...
class HyakkiyakouConfig(ConfigBase):
    switch_onmyoji_enable: bool = Field(default=False, description='是否切换阴阳师')
    hya_onmyoji: Onmyoji = Field(default=Onmyoji.KAGURA, description='阴阳师类型')
...
class HyakkiyakouModels(ConfigBase):
    use_new_model: bool = Field(default=True, description='是否使用新模型\n新模型只有 SR SSR SP UR')
    # 置信度阈值
