...
class BackUp:
    ...
    # 中文星期映射（避免使用 locale.setlocale 修改全局区域设置）
    WEEKDAY_CN = ['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日']

    def run(self):
        logger.hr('BackUp start', 2)
        try:
            # 检查并创建delete目录
            ...
            logger.error(f'BackUp 异常: {e}')
            logger.error(traceback.format_exc())
        logger.hr('BackUp end', 2)
            ...
            for file_name in files:
                ...
                try:
                    ...
                    if time_diff >= days_threshold:
                        # 构建动态备份路径（按年-月-日组织，使用手动星期映射避免 locale 问题）
                        weekday_cn = self.WEEKDAY_CN[file_time.weekday()]
                        date_str = f"{file_time.strftime('%Y-%m-%d')} {weekday_cn}"
                        relative_path = os.path.relpath(root, base_path)  # 保留相对路径
                        backup_root = os.path.join(back_dir, date_str, relative_path)
                        # 检查并创建备份目录
                ...
                try:
                    ...
                    try:
                        ...
                        if os.path.exists(dest):
                            logger.warning(f"目标已存在，先清理：{dest}")
