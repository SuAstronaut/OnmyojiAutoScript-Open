...
from module.exception import BigGodSignInError
...
class BigGodSignIn:
    ...
    def start_sign_in(self, serial):
        ...
        if not pid:
            ...
            raise BigGodSignInError('无法启动大神APP，请确保模拟器中已安装大神APP')
        self.frida_pid = pid
        ...
        if not token_data:
            ...
            raise BigGodSignInError('无法获取Token，请确保已登录大神APP并绑定阴阳师角色')
        ...
        self._cleanup()
        logger.hr('BigGodSignIn end', level=2)
...
if __name__ == '__main__':
    ...
    config = Config('mi')
    serial = config.script.device.serial
    BigGodSignIn().start_sign_in(serial=serial)
